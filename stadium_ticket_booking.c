...
//struct seat: Represents a seat with information about its row, seat number, and a pointer to the next seat (for circular queue implementation).
struct Seat {
    int ROW;
    int seatNUM;
    struct Seat* next;
...
//The circular queue is implemented using a linked list of seats, where the last seat points to the first one. This helps efficiently manage seat bookings and cancellations.